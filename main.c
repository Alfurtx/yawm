...
#define WINDOWMASKS                                                         \
	(SubstructureRedirectMask | SubstructureNotifyMask | KeyPressMask | \
	 ...
	 PointerMotionMask)
#define MODMASK(mask)                                                       \
	(mask & (ShiftMask | LockMask | ControlMask | Mod1Mask | Mod2Mask | \
...
internal int xiniterror(Display *display, XErrorEvent *error)
{
	if (error->error_code == BadAccess) {
		die("Another Window Manager is already active",
		    error->error_code);
		exit(EXIT_FAILURE);
	}
...
internal void rearrange(void)
{
	for (uint i = 0; i < arrlenu(manager.monitors); i++) {
		...
		if (!count)
			...
			resizeclient(&monitor->clients[0], monitor->x_orig,
				     monitor->y_orig,
				     monitor->width - 2 * border_width,
				     monitor->height - 2 * border_width,
				     border_width);
		else {
			...
			resizeclient(c, x, y, ws - 2 * bw,
				     monitor->height - 2 * bw, bw);
			...
			for (uint i = 0; i < count - 1; i++) {
				...
				if (i == 0) {
					resizeclient(c, x, y, ws - 1 * bw,
						     hs - 2 * bw, bw);
				} else if (i == count - 2) {
					resizeclient(c, x, y - bw, ws - 1 * bw,
						     hs - bw + excess, bw);
				} else {
					resizeclient(c, x, y - bw, ws - 1 * bw,
						     hs - 1 * bw, bw);
...
internal void grabkeys(void)
{
	...
	for (uint i = 0; i < ARRLEN(keys); i++) {
		if ((code = XKeysymToKeycode(display, keys[i].keysym))) {
			XGrabKey(display, code, keys[i].modifiers, root, True,
...
internal void focus(Window window)
{
	...
	for (uint i = 0; i < arrlenu(monitor->clients); i++) {
		if (monitor->clients[i].window != window) {
			XSetWindowBorder(display, monitor->clients[i].window,
...
internal void setupatoms(void)
{
	...
	netatoms[NET_WM_ACTIVE_WINDOW] =
		XInternAtom(display, "_NET_ACTIVE_WINDOW", False);
	netatoms[NET_WM_WINDOW_TYPE] =
		XInternAtom(display, "_NET_WM_WINDOW_TYPE", False);
	netatoms[NET_WM_CLIENT_LIST] =
	...
	for (uint i = 0; i < moncount; i++) {
		...
		if (clientcount > 0) {
			for (uint j = 0; j < clientcount; j++) {
				if (manager.monitors[i].clients[j].window ==
				    window)
					return (i);
			}
		}
...
{
	for (uint i = 0; i < arrlenu(manager.monitors); i++) {
		for (uint j = 0; j < arrlenu(manager.monitors[i].clients);
		     j++) {
			if (manager.monitors[i].clients[j].window ==
			    client->window)
				return (&manager.monitors[i]);
...
internal uint coordstomonitorpos(uint x, uint y)
{
	for (uint i = 0; i < arrlenu(manager.monitors); i++) {
		...
		if (x > mon->x_orig && x < mon->x_orig + mon->width &&
		    y > mon->y_orig && y < mon->y_orig + mon->height)
			return (i);
	}
	return (0);
}
